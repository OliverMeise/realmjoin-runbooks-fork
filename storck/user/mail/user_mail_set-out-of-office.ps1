<#
  .SYNOPSIS
  En-/Disable Out-of-office-notifications for a user/mailbox (Storck).
...
try {
    ...
    if ($Disable) {
        ...
        if ($MessageExternal) {
            ...
            Set-MailboxAutoReplyConfiguration -Identity $UserName -AutoReplyState Scheduled -ExternalMessage $MessageExternal -InternalMessage $MessageInternal -StartTime $Start -EndTime $End -CreateOOFEvent $CreateEvent -OOFEventSubject $EventSubject
        } else {
            "## Enabling Out Of Office settings for '$UserName'"
            Set-MailboxAutoReplyConfiguration -Identity $UserName -AutoReplyState Scheduled -InternalMessage $MessageInternal -StartTime $Start -EndTime $End -CreateOOFEvent $CreateEvent -OOFEventSubject $EventSubject
        }
    }
